...
/*   Updated: 2026/02/21 15:11:45 by ykonka           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void draw(t_set_params *set_params)
{
	...
	while (x_a < set_params->img->width)
	{
		...
		{
			set_params->z.real = transform_pixels_to_pattern_coordinates(x_a, &(set_params->plane_params.x));
			set_params->z.imaginary = transform_pixels_to_pattern_coordinates(y_b, &(set_params->plane_params.y));
			iteration = 0;
...
void julia(t_set_params *set_params, double ca, double cb)
{
	...
	set_params->c.real = ca;
	set_params->c.imaginary = cb;
	draw(set_params);
}
